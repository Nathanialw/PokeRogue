...
typedef union
{
    uint16_t frameBuffer[BUFFER_SIZE * 2];
    uint8_t frameBuffer1byte[BUFFER_SIZE * 4];

    struct
    {
        uint16_t front[BUFFER_SIZE];
        uint16_t back[BUFFER_SIZE];
    };
} PartialFrameBuffer;

_Static_assert(sizeof(PartialFrameBuffer) == 25088, "SpriteLayout must be 25088 bytes");
...
_Static_assert(sizeof(State) == 1, "State must be 1 byte");
...
_Static_assert(sizeof(MusicData) == 56, "MusicData must be 56 bytes");
