...
int SetUseEntity(std::vector<std::string>& image_folders, uint16_t& entity_count, const EntityTypes& entity_type)
{
    // 1. Open the database (must already exist)
    sqlite3* db = nullptr;
    int rc = sqlite3_open(db_path.c_str(), &db);
...
int SetGenerateImage(std::vector<std::string>& image_folders, uint16_t& entity_count, const EntityTypes& entity_type)
{
    // 1. Open the database (must already exist)
    sqlite3* db = nullptr;
    int rc = sqlite3_open(db_path.c_str(), &db);
